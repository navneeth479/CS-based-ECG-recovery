...
phantom = utils.dotsgrid2D(128, 128)
...
sinogram = recon.create_sinogram(grid_phantom.get_buffer(), 200, 1, 200, np.pi)
...
recon_image, library = recon.backproject(filtered_sino, grid_phantom.get_size()[0], grid_phantom.get_size()[1],
                                         grid_phantom.get_spacing())
utils.show(np.rot90(library, k=-1), "FBP image ")
...
start_time = time.time()
img_opencl = recon.backprojectOpenCL(filtered_sino, grid_phantom.get_size()[0], grid_phantom.get_size()[1],
                                     grid_phantom.get_spacing())
utils.show(recon_image, "OpenCL recon image")
print("GPU accelerated backprojection run time(s) : %s" % (time.time() - start_time))

utils.show(phantom - np.rot90(library, k=-1), "Phanton - FBP difference")
utils.show(phantom - recon_image, "Phanton - OpenCL FBP difference")
